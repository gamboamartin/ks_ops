...
class em_empleado extends \gamboamartin\empleado\models\em_empleado {
    ...
    public function elimina_bd(int $id): array|stdClass
    {
        ...
        $filtro['em_empleado.id'] = $id;

        $r_empleado_documento = (new em_empleado_documento(link: $this->link))->elimina_con_filtro_and(filtro: $filtro);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al eliminar empleado_documento', data: $r_empleado_documento);
        }

        $r_empleado = (new ks_empleado(link: $this->link))->elimina_con_filtro_and(filtro: $filtro);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al eliminar extension de empleado', data: $r_empleado);
        }

        $r_elimina_bd = (new ks_cliente_empleado(link: $this->link))->elimina_con_filtro_and(filtro: $filtro);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al eliminar cliente_empelado', data: $r_elimina_bd);
        }

        $r_elimina_bd = parent::elimina_bd(id: $id); // TODO: Change the autogenerated stub
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al eliminar xxxxxxxxxx', data: $r_elimina_bd);
        }
        return $r_elimina_bd;
    }
